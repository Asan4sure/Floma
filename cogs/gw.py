...
class Giveaways(commands.Cog):
    ...
    async def _giveaway(self, ctx):
        ...
        **Shows the giveaway commands
        ...
        """
        em = discord.Embed(title="__**Giveaway Commands**__", color=0x2f3136)
        em.add_field(
            name="`-gstart <time> <winners> <message/prize>`",
    ...
    async def _gstart(self, ctx, timee: str, winners: str, *, message):
        ...
        **Starts the giveaway**
        ...
        em = discord.Embed(
            title=f'{message}',
            description=
            f"<:zzz_dot:1210596527731507290> **Duration: {timee}**\n<:zzz_dot:1210596527731507290> **Hosted by: {ctx.author.mention}**\n<:zzz_dot:1210596527731507290> **Winners: {winners}**",
            color=0x2f3136)
        # end  = datetime.datetime.utcnow() + datetime.timedelta(seconds=time)
        end = datetime.datetime.utcnow() + datetime.timedelta(seconds=time)
        em.set_footer(text=f"Ends at • {end}")
        ...
        msg = await ctx.send(
            "<a:giveaways:1208688340736409670> **GIVEAWAY** <a:giveaways:1208688340736409670> ",
            embed=em)
        #print(msg)
        gchannel = ctx.channel
        await msg.add_reaction("<a:giveaways:1208688340736409670>")
    ...
    async def gend(self, msg, em, winners, message, gchannel, end):
        ...
        for reaction in cache_msg.reactions:
            if str(reaction.emoji) == "<a:giveaways:1208688340736409670>":
                users = await reaction.users().flatten()
                #print(reaction.users())
                if len(users) == 1:
                    await msg.edit(
                        content=
                        "<a:giveaways:1208688340736409670>    **GIVEAWAY ENDED**    <a:giveaways:1208688340736409670>"
                    )
        ...
        try:
            ...
            x += f"** You won {message}** <a:giveaways:1208688340736409670>"
